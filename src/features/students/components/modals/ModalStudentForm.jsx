...
export default function ModalStudentForm({
  modal,
  onClose,
  onAudit,
  onMutate,
  ...
  const [class_id, setClassId] = useState(
    current?.class_id || MOCK_CLASSES[0]?.id,
  );
  const [fingerprint_id, setFP] = useState(
    (current?.fingerprint_id || "").trim(),
  );

  const err = !student_code.trim()
    ? "Thiếu mã học sinh"
    : !full_name.trim()
      ? "Thiếu họ tên"
      : !class_id
      ...
      <div className="stu-modal" onClick={(e) => e.stopPropagation()}>
        <div className="stu-modalHead">
          <div className="strong">
            {isAdd ? "Thêm học sinh" : "Sửa học sinh"}
          </div>
        ...
        <div className="stu-modalBody">
          ...
          {err ? (
            <div
              className="muted"
              style={{ marginTop: 10, color: "rgba(239,68,68,.95)" }}
            >
              {err}
            </div>
          ) : null}
